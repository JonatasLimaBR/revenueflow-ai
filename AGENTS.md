# AGENTS.md — RevenueFlow AI

## Harness oficial
O harness oficial é **Codex**. Cursor pode ser usado como editor, mas este arquivo é a fonte autoritativa de contexto para agentes de código.

## Missão
Construir o **RevenueFlow AI — Autonomous WhatsApp Sales Platform** em GCP, com WhatsApp como primeiro canal e autonomia proporcional ao risco.

## Invariantes
1. LLM nunca é System of Record para preço, estoque, margem, cliente, pedido ou pagamento.
2. **LLM interpreta → Policy Engine decide → API executa.**
3. Outbound sempre passa por Policy Gate, consentimento e frequency cap.
4. Pagamento é somente sandbox na V1.
5. Documento fiscal/contratual real está fora da V1.
6. PII deve ser minimizada.
7. Cada agente recebe apenas as tools necessárias.
8. Webhook, quote, order e payment devem ser idempotentes.
9. Mudança arquitetural exige ADR.
10. Remover guardrail para fazer teste passar é proibido.
...
```text
docs/prd/           # produto
docs/specs/         # comportamento implementável
docs/adrs/          # decisões de arquitetura
docs/engineering/   # processo e governança
src/revenueflow/    # código
tests/unit/
tests/integration/
tests/ai_eval/
tests/security/
...
3. Leia os ADRs relacionados.
4. Identifique risco e operações irreversíveis.
5. Defina testes.
6. Só então implemente.

Se a SPEC não permitir implementar sem adivinhar, proponha uma SPEC; não invente requisito.

## O agente pode
- implementar comportamento já especificado;
- criar testes e mocks;
- refatorar sem mudar contrato;
- corrigir lint/tipos;
- atualizar documentação coerente com a mudança;
- propor ADRs.

## O agente não pode
- alterar pricing/margem por conta própria;
- remover aprovação humana;
- criar pagamento real;
- disparar outbound sem Policy Gate;
- ampliar permissões de tools sem justificativa;
- enviar mais PII ao LLM;
- trocar cloud, banco ou arquitetura sem ADR;
- fazer merge na main;
- ignorar checks.

## Regras de arquitetura
- `domain` não depende de FastAPI, GCP ou LLM.
- Regra de negócio vive em `domain`/`policies`, não em endpoint.
- Agente não acessa banco diretamente.
- Tool chama service/repository autorizado.
- Repository não conhece LLM.
- Prompt não contém secrets.
- Logs são estruturados e carregam `trace_id`.

## Testes obrigatórios
### Unit
pricing, margem, policy gate, lead scoring, idempotência, autorização.

### Integration
WhatsApp adapter, PostgreSQL, Pub/Sub, BigQuery, tools, order flow.

### AI Eval
intent, recomendação, tool selection, recusa de inventar preço/estoque, handoff.

### Security
prompt injection, privilege escalation, cross-customer leakage, secret leakage, forbidden tools.

## Definition of Done
- PRD/SPEC/ADR identificados no PR;
- lint, typecheck e testes verdes;
- testes de segurança relevantes verdes;
- sem secrets;
- docs atualizadas;
- ADR para mudança arquitetural;
...
Não confie em frases como "não use esta tool".

Se um agente não pode executar uma ação, a tool correspondente **não deve ser registrada** para esse agente.
...
Nenhum agente recebe uma tool genérica de `set_discount`.

Ações irreversíveis devem passar por `interrupt`/checkpoint persistente quando a política exigir aprovação.

## Ritual de revisão independente

Antes do merge, uma sessão diferente da sessão autora deve executar `.codex/commands/verificar-spec.md`.

O revisor:
- pode ler;
- pode executar testes;
- pode emitir veredito;
- **não pode corrigir o código na mesma sessão de revisão**.

## Catálogo obrigatório de PRDs, SPECs e ADRs

Antes de implementar qualquer mudança, o agente deve localizar os documentos relacionados neste catálogo. Estes arquivos são parte do contexto obrigatório do projeto.
...
Se houver conflito:
1. segurança e invariantes do `AGENTS.md`;
2. ADR aceito;
3. SPEC;
4. PRD;
5. código existente.

O agente não pode ignorar um documento porque a implementação atual faz algo diferente.
