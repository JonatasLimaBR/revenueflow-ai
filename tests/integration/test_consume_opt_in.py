...
from revenueflow.worker.consume import _OPT_IN_CONFIRMED
...
def graph_spy(outbound: FakeOutbound, monkeypatch: pytest.MonkeyPatch) -> None:
    async def _boom(*_args: Any, **_kwargs: Any) -> Any:
        raise AssertionError("graph.ainvoke should not run for an opt-in message")

    monkeypatch.setattr(get_graph(), "ainvoke", _boom)


async def _create_customer(phone: str) -> str:
    customer_id = f"CUST-OPTIN-{uuid4().hex[:8]}"
...
@pytest.mark.parametrize(
    "text", ["ACEITO RECEBER OFERTAS", " Aceito receber ofertas ", "aceito receber ofertas."]
)
async def test_opt_in_persists_and_replies_without_graph(
    text: str, db: None, graph_spy: None, outbound: FakeOutbound
) -> None:
    phone = f"+5511{uuid4().hex[:9]}"
    customer_id = await _create_customer(phone)

    env = _envelope(phone, text, f"e-optin-{uuid4().hex[:8]}")
    assert await process_event(env, outbound=outbound) is True

    assert len(outbound.sent) == 1
    assert outbound.sent[0]["text"] == _OPT_IN_CONFIRMED

    async with unit_of_work() as conn:
        stored = await customer_repo.get_by_id(conn, customer_id)
    assert stored is not None
    assert stored.consent_opt_in_at is not None


async def test_opt_in_false_positive_runs_graph_normally(db: None, outbound: FakeOutbound) -> None:
    phone = f"+5511{uuid4().hex[:9]}"
    env = _envelope(phone, "sim, aceito o desconto", f"e-optin-fp-{uuid4().hex[:8]}")
...
async def test_opt_in_unknown_lead_no_customer_update(
    db: None, graph_spy: None, outbound: FakeOutbound
) -> None:
    phone = f"+5511{uuid4().hex[:9]}"
    env = _envelope(phone, "aceito receber ofertas", f"e-optin-lead-{uuid4().hex[:8]}")
