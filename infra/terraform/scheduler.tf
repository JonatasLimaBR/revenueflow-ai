...
# OPPORTUNITY_ENGINE/ACTIVE_SALES — os 4 Jobs só rodavam sob demanda via
...
# analytics_sync em horários independentes.
#
# SA dedicada com escopo mínimo (ADR-008): só roles/run.invoker nos 4 Jobs
...
resource "google_cloud_scheduler_job" "this" {
  ...
  name      = "${var.service_name}-${replace(each.key, "_", "-")}-daily"
  region    = var.region
